...
tracked_device name -- gary_iphone 
...
#### Using IOS App -- Differences between Version 1 and Version 2

iCloud3 supports both version 1 and version 2 of the IOS App. However, there are some important differences between the versions in the way the device_tracker entities are used. The name associated with the device, as described above, is used by iCloud3 to tie iCloud's Find-my-Friend or Find-my-Phone Location Service to the device being tracked. You also have to associate the device's device name to the device running the IOS App.

**Version 1**

The same device_tracker entity is used by the iPhone, iPad or iWatch and the IOS App. It is entered on the IOS App's Settings page in the 'Device ID' field. All information (location, battery status, action triggers, etc.) are updated by the IOS App and iCloud in this shared device_tracker entity. For example,  the IOS App will issue Zone enter and exit notifications and location updates and enter this information in `device_tracker.gary_iphone` . iCloud will also update it's information in `device_tracker.gary_iphone`. iCloud3 monitors `device_tracker.gary_iphone` and uses this information to track the device. 

**Version 2 -- Installed after iCloud3, with a prior IOS App v1 installation or when a number is added to the device name.**

Version 2 is a lot more complicated. Unlike Version 1, you can not specify the device's device name within the IOS App and it does not use a shared device_tracker entity. When the IOS App is added to the device, a device_tracker entity is added to HA and assigned a unique name. For example, if `device_tracker.gary_iphone` exists (iCloud3 is already installed or you are upgrading from version 1), a new name is assigned (`device_tracker.gary_iphone_2`). If you have problems and need to delete and reinstall the IOS App, the '_2' entity exists so the suffix number is incremented and a new device entity is created (`device_tracker.gary_iphone_3`).  Full documentation for IOS App version 2 is [here](https://companion.home-assistant.io/en/getting_started/).

The suffix number (_2, _3, etc.) is entered on the *tracked_device* configuration parameter and used to tie the device to the device_tracker entity associated with the iCloud Location Services device. iCloud3 monitors the `device_tracker.gary_iphone_2` entity for IOS App information and `device_trcker.gary_iphone` for Find-my-Friends or Find-my-Phone information. iCloud3 updates the `device_tracker.gary_iphone` entity.

An example of the track_devices parameter showing this is `- gary_iphone > gary-2fa-acct@email.com, gary.png, 2`. Parameter#3 (the #2) is the IOS App suffix number and further described below in the Configuration Parameters section.

Version 2 also creates many sensor entities and may or may not add a suffix number to the entity name. Unfortunately, there is no correlation between the number added to the device_tracker and the sensor. For example, the IOS App issues trigger notifications to indicate if a zone has been entered, exited, significant location update and others. The same device may use `device_tracker.gary_iphone_2` and `sensor.last_update_trigger_3`. If you add several devices at the same time, you end up with `sensor.last_update_trigger_1`, `sensor.last_update_trigger_2`, `sensor.last_update_trigger_3`, etc. and it can be a challenge to tell one from another. It is best to set up one device at a time.

iCloud3 monitors the `sensor.<devicename>_last_update_trigger` entity for zone enter, exit, significant location update and other triggers ([more info here](https://companion.home-assistant.io/en/core/sensors#last-update-trigger-sensor)). For example, it will monitor `sensor.gary_iphone_last_update_trigger` to see when it exits a zone. The sensor's entity name must be changed to tie it to the device (`sensor.last_update_trigger_2  to  sensor.gary_iphone_last_update_trigger`). This is done using the Integrations window on the HA Configuration page. The steps to do this are described in the IOS App version 2 documentation and are repeated below.

```
I am running the IOS App on multiple devices, the sensor names are too similar and confusing, what can I do?
...
3. Find the "Mobile App " integration corresponding the device you wish to rename the sensors of and open it
...
**Version 2 - Installed before iCloud3, without a prior IOS App v1 installation or when a number is not added to the device name.**

If you have not previously installed iCloud3 or the IOS App v1, the device's devicename may not be in the *known_devices.yaml* file and a number may not be added to the end of the entity name when v2 is installed on the device. For example, the device_tracker entity is`device_tracker.gary_iphone`. In this case, use `iosappv2` instead of the suffix number on the tracked_devices configuration parameter (more information below). The device on the *tracked_devices* parameter will then be tied the device running IOS App v2  and the iCloud Location Services. 

An example of the *tracked_devices* parameter is: `- gary_iphone > gary-2fa-acct@email.com, gary.png, iosappv2`
...
device_tracker:
  - platform: icloud3
    ...
    track_devices:                     <<< Devices to be tracked
      - gary_iphone > gary-2fa-acct@email.com, gary.png, 2
...
device_tracker:
  - platform: icloud3
    ...
    track_devices:                   <<< Devices to be tracked
      - gary_iphone > gary-2fa-acct@email.com, gary.png, 2
      - lillian_iphone > lillian-2fa-acct@email.com, lillian.png, 3
...
​    
    ...
    devicename > badge_picture_name, 'iosapp2'
...
| ----------------------------- | ------------------------------------------------------------ |
| email_address                 | FmF uses the "real" email address of your "friends" iCloud account to link it with the FmF account, i.e., `gary-2fa-acct@email.com`.  <br><br>*Note:* This parameter is identified as the *email_address* by the '@' character. It is required for Find-my-Friends tracking method and not used by the other tracking methods and can be omitted. |
| badge_picture_name            | The file name containing a picture of the person normally associated with the device. See the sensor_badge section of the iCloud3 documentation for more information about the sensor_badge.<br>The file is normally in the ```config/www/``` directory (referred to as '/local/' in HA). You can use the full name ```(/local/gary.png)``` or an abbreviated name ```(gary.png)```.<br><br>*Note:* This parameter is identified as the *badge_picture_name* since it ends in '.png' or '.jpg'. |
| IOS App Id Number (version 2) | At the present time, the IOS App version 2 does not provide any means of identifying the physical device with the device being tracked. A number '_2', ' _3', etc. is added to the device_tracker entity_id to make the attribute name unique.<br><br>If you have not previously installed iCloud3 or the IOS App v1, a number may not be added to the end of the entity name when v2 is installed on the device. In this case, use `iosappv2` instead of a number to tie the IOS App device_tracker entity to iCloud3 and the iCloud Location Services.<br><br>Adding iCloud3 This field is used to specify device's Id number assigned by the IOS App version 2. This field can be omitted if you are using version 1.<br><br>*Note:* This parameter is identified as the *IOS App ID Number* since it is a number. |
| sensor_name_prefix            | See the Sensor section of the documentation for a complete description of this field. (garyc in the example below)<br><br>*Note:* This parameter is identified as the *sensor_name_prefix* since it is none of the above. |
...
This service will play the Lost iPhone sound on a specific device when you use the Find-my-Phone tracking method. Due to an Apple limitation, this is not available for the Find-my-Friends tracking method.
