...
VERSION = '2.0b'      #Custom Component Updater
...
ATTR_BATTERY_STATUS_FMF = 'batteryStatus'
...
TRACE_ATTRS_BASE        = {ATTR_ZONE: '', ATTR_LAST_ZONE: '',
                           ...
                           ATTR_WAZE_DISTANCE: '', ATTR_CALC_DISTANCE: 0,
                           ATTR_LAST_LOCATED: '',
...
def setup_scanner(hass, config: dict, see, discovery_info=None):
    ...
    #        group = account_name.lower()
    #        log_msg =("Using depreciated 'account_name' config parameter '{}'. "
    #            "Should use 'group' parameter").format(group)
    #        _LOGGER.warning(log_msg)
    #    else:
    #        group = base_zone.lower()
...
class Icloud(DeviceScanner):
    ...
    def __init__(self,
        ...
        self._initialize_debug_control()
        ...
        self.old_location_threshold      = self._time_str_to_secs(old_location_threshold_str) * 60
        OLD_LOCATION_THRESHOLD           = self._time_str_to_secs(old_location_threshold_str) * 60
        ...
        self._initialize_um_formats(unit_of_measurement)
        ...
        self._define_stationary_zone_fields(stationary_inzone_interval_str,
                    stationary_still_time_str)
        self._verify_waze_installation()
        self._initialize_waze_fields(waze_region, waze_min_distance,
...
#--------------------------------------------------------------------
    def restart_icloud(self):
        ...
        #if self.group == self.account_name:
        #    event_msg = ("Group not specified for {}, using retired "
        #        "`account_name` parameter <{}> for `group`").format(
        #        self.username,
        #        self.group)
        #    self._save_event('*', event_msg)
        ...
        if (PYICLOUD_IC3_IMPORT_SUCCESSFUL is False and \
            ...
            event_msg = ("Error: Falling back to IOSAPP instead of {}").format(self.trk_method_s)
            self._save_event_halog_error("*", event_msg)
        ...
        if self.CURRENT_TRK_METHOD_IOSAPP:
            ...
            try:
                ...
                event_msg = ("Error authenticating iCloud FmF/FamShr Service "
                    "for Username: {}, Group: {}. iCloud may be down or the "
        ...
        try:
            ...
            for devicename in self.devicename_verified:
                ...
                if self.devicename_verified.get(devicename):
                    ...
                    if self.devicename_iosapp_id.get(devicename) != '':
                        self.track_devicename_list = '{} ({})'.format(
                            self.track_devicename_list,
                            self.devicename_iosapp_id.get(devicename).replace(devicename,'',2))
                    event_msg_prefix = ""
            ...
            if self.track_devicename_list == '':
                event_msg = ("Error: iCloud3 setup aborted for group '{}', "
            ...
            for devicename in self.tracked_devices:
                ...
                self._update_event_log_sensor_line_items(devicename)
    ...
    def _polling_loop_5_sec_device(self, now):
        ...
        if count_reset_timer == '00:00:05':
            for devicename in self.tracked_devices:
                ...
                event_msg = ("^^^ {}, Group: {}, Zone: {} ^^^").format(
                    dt_util.now().strftime('%a, %b %d'),
                    self.group,
                    self.base_zone)
                self._save_event(devicename, event_msg)
                if self.devicename_iosapp_id.get(devicename) != '':
                    event_msg = "IOS App v2 monitoring > device_tracker.{}".format(
                        self.devicename_iosapp.get(devicename))
                    self._save_event_halog_info(devicename, event_msg)
                    event_msg = "IOS App v2 monitoring > sensor.{}".format(
                        self.iosapp_v2_last_trigger_entity.get(devicename))
                    self._save_event_halog_info(devicename, event_msg)
            ...
            for devicename in self.tracked_devices:
                ...
                if self.iosapp_version.get(devicename) == 2:
                    ...
                    if self.last_v2_trigger.get(devicename) == '':
                        ...
                        self.last_v2_tigger_changed_time[devicename] = v2_trigger_changed_time
                        self.last_v2_tigger_changed_secs[devicename] = v2_trigger_changed_secs
                    ...
                    elif (v2_trigger != self.last_v2_trigger.get(devicename) or \
                            v2_trigger_changed_secs > self.last_v2_tigger_changed_secs.get(devicename)):
                        update_via_v2_flag = True
                    ...
                    if update_via_v2_flag:
                        ...
                        dev_data[ATTR_LATITUDE]  = round(v2_dev_attrs[ATTR_LATITUDE], 6)
                        dev_data[ATTR_LONGITUDE] = round(v2_dev_attrs[ATTR_LONGITUDE], 6)
                ...
                #latitude and longitude. Reset via icloud update.
                    ...
                    self.next_update_secs[devicename] = 0
                ...
                #zone in error. Try to reset to the last zone location.
                    ...
                    if (self.last_lat.get(devicename) == 90 or
                            self.last_long.get(devicename) == 180):
                        self.restart_icloud_group_request_flag = True
                        
                        log_msg = ("Error: Could not recover from device "
                            "location error. Restarting iCloud3")
                        self._save_event_halog_error(devicename, log_msg)
                    else:
                        update_via_icloud_flag = True
                    
                        log_msg = ("Error: IOS App entered initial Stationary Zone. "
                            "Resetting to previous location ({}:{}) to ({}:{}). "
                            "Will try to correct.").format(
                ...
                if update_via_iosapp_flag:
                    ...
                    if (dev_trigger in
                        ...
                        if update_via_ios_flag:
                            event_msg = "{} trigger detected"
                            self._save_event_halog_info(devicename, event_msg)

                            self.last_iosapp_trigger[devicename] = dev_trigger
                            distance = self._current_zone_distance(
                                devicename,
                                dev_latitude,
                                dev_longitude)

                            #set to last gps if in a zone
                            if distance < self.zone_home_radius * 1000:
                                event_msg = ("Moving back to zone <{}> center, "
                            ...
                            #discard if between 0-1km away
                                ...
                                event_msg = ("Discarding, keeping in zone <{}>, "
                                    "No ExitZone trigger detected, "
                                    "distance {}m").format(
                                    current_state,
                                    distance)
                                self._save_event_halog_info(devicename, event_msg)
                ...
                if devicename in self.track_from_zone:
                    for zone in self.track_from_zone.get(devicename):
                        ...
                        if devicename_zone in self.next_update_secs:
                            #age_secs = self.next_update_secs.get(devicename_zone) - \
...
#########################################################
    def _update_device_iosapp_trigger(self, update_reason, devicename, dev_data):
        ...
        try:
            ...
            if ATTR_ALTITUDE in dev_data:
                altitude       = dev_data[ATTR_ALTITUDE]
            else:
                altitude = 0
    ...
    def _polling_loop_15_sec_icloud(self, now):
        ...
        try:
            ...
            for devicename in self.tracked_devices:
                ...
                if self._check_update_but_in_zone(devicename):
                    continue
                ...
                elif self._check_next_update_time_reached():
                    update_device_flag = True

                    self.log_debug_msgs_trace_flag = False
                    update_reason = ("NextUpdate time reached for {}").format(
                        devicename)
                    self._save_event('*', update_reason)
            ...
            for devicename in self.tracked_devices:
                ...
                if dev_data[0] is False:
                    ...
                    if self.icloud_acct_auth_error_cnt > 20:
                        ...
                        log_msg = ("Error: More than 20 iCloud authoriation "
                ...
                #Discard if location is too old
                    ...
                    age_str = self._secs_to_time_str(location_age)
...
#########################################################
    def _get_fmf_data(self, devicename, refresh_fmf_location_data):
        ...
        try:
            ...
            if self._get_fmf_updated_location_data() is False:
                ...
                try:
                    ...
                    _LOGGER.exception(err)
                    self.log_error_msg("Error: Initial reauthorization failed")
                    return ICLOUD_LOCATION_DATA_ERROR

            #Successful authentication, get data again and recheck
            if self._get_fmf_updated_location_data() is False:
                self.log_error_msg("Error: Retry reauthorization failed")
                return ICLOUD_LOCATION_DATA_ERROR
            ...
            self.log_error_msg("Error: FmF Location Data Error")
            _LOGGER.exception(err)
            return ICLOUD_LOCATION_DATA_ERROR

        try:
            if devicename not in self.fmf_location_data:
                log_msg = ("Error: Devicename {} is not in location data: {}").format(
                    devicename,
            ...
            if location_data is None:
                if self.icloud_acct_auth_error_cnt > 3:
                    self.log_error_msg("Error: No FmF Location Data Returned for {}").format(
                        devicename)
                return ICLOUD_LOCATION_DATA_ERROR
            ...
            if location is None:
                if self.icloud_acct_auth_error_cnt > 3:
                    self.log_error_msg("Error: No FmF Location Data Returned")
                return ICLOUD_LOCATION_DATA_ERROR

            battery_status = ''
            battery        = 0
            if ATTR_BATTERY_STATUS_FMF in location:
                battery_status = location.get(ATTR_BATTERY_STATUS_FMF)
...
#########################################################
    ...
    def _get_distance_data(self, devicename, latitude, longitude,
        ...
        try:
            ...
            if calc_dist_from_zone < .05 or current_zone == self.base_zone:     #HOME
            ...
            #Near home & towards or in near_zone
                ...
                self.log_debug_interval_msg(devicename, log_msg)
                event_msg = ("__Using Calc method, near Home or Waze off")
                #self._save_event(devicename, event_msg)
            ...
            if self.waze_status == WAZE_USED:
                try:
                    ...
                    if self.waze_status != WAZE_ERROR:
                        ...
                        event_msg = ("__Waze request successful, distance {}{}, "
                                "time {} min").format(
    ...
    def _setup_base_kwargs(self, devicename, latitude, longitude,
        ...
        if zone_name:
            if self.zone_latitude.get(zone_name):
                ...
                self.last_lat[devicename]  = latitude
                self.last_long[devicename] = longitude
                log_msg = ("Overiding GPS for {} with {}, {}/{}").format(
...
#--------------------------------------------------------------------
    def _get_iosappv2_device_sensor_trigger(self, devicename):
        ...
        try:
            if self.iosapp_version.get(devicename) == 2:
                #dev_attrs   = self.hass.states.get(entity_id)
    ...
    def _format_info_attr(self, devicename, battery,
        ...
        try:
            ...
            if self.log_debug_msgs_flag:
                info_msg = '{} ●Debug.log-on'.format(info_msg)
            ...
            poll_cnt = self.poll_count_icloud.get(devicename) + \
            ...
            if poll_cnt > 0 and (poll_cnt % 5) == 0:
                 info_msg = '{} ●Poll.Cnt: iCloud-{}, IOSApp-{}, Ignore-{}'.format(
                    info_msg,
...
#--------------------------------------------------------------------
    def _define_device_status_fields(self):
        ...
        self.last_v2_tigger_changed_time = {}
        self.last_v2_tigger_changed_secs = {}
...
#--------------------------------------------------------------------
    def _initialize_debug_control(self):
...
#--------------------------------------------------------------------
    def _initialize_device_fields(self, devicename):
        ...
        self.last_v2_tigger_changed_time[devicename] = ''
        self.last_v2_tigger_changed_secs[devicename] = 0
    ...
    def _setup_tracked_devices_config_parm(self, config_parameter):
        ...
        try:
            for track_device_line in config_parameter:
                di = self._setup_track_device_config_parm_decode(
...
#--------------------------------------------------------------------
    def _setup_track_device_config_parm_decode(self,
                track_device_line, iosapp_v2_entities):
        '''
        ...
        try:
            ...
            badge_picture = None
            fname         = None
            iosappv1_flag = (iosapp_v2_entities is [])
            iosappv2_id   = ''
            device_type   = None
            sensor_prefix = None
            zones         = []
            ...
            log_msg = ("Decoding track_devices <{}>").format(
                track_device_line)
            self.log_info_msg(log_msg)
            ...
            dt_devicename = 'device_tracker.{}{}'.format(devicename, iosappv2_id)
            device_id     = None
            ...
            if iosappv1_flag is False:
                for entity in iosapp_v2_entities:
                    if (entity['entity_id'].startswith(dt_devicename)):
                        self.iosapp_version[devicename] = 2
                        device_id            = entity['device_id']
                        devicename_iosapp    = entity['entity_id'].replace('device_tracker.', '', 5)
                        devicename_iosapp_id = devicename_iosapp.replace(devicename, '', 5)

                if device_id is not None:
                    for entity in iosapp_v2_entities:
                      if (entity['device_id'] == device_id and
                            entity['entity_id'].startswith('sensor.') and
                            entity['entity_id'].endswith('last_update_trigger')):
                          sensor_last_trigger_entity = entity['entity_id'].replace('sensor.', '', 5)

            device_info = [devicename, device_type, fname, email, badge_picture,
                           devicename_iosapp, devicename_iosapp_id,
...
#########################################################
    def _check_location_isold(self, devicename, arg_location_isold_flag, timestamp_secs):
        ...
        try:
            ...
            location_isold_flag = (age > OLD_LOCATION_THRESHOLD)
...
#--------------------------------------------------------------------
    def _check_next_update_time_reached(self, devicename = None):
        ...
        '''
        try:
            if self.next_update_secs is not None:
                for devicename_zone in self.next_update_secs:
                    if (devicename is None or devicename_zone.startswith(devicename)):
                        ...
                        if time_till_update <= 0:
                            return True
                        #elif (time_till_update < 180):
                        #    self._display_time_till_update_info_msg(
                        #            time_till_update, devicename_zone)
        except Exception as err:
            _LOGGER.exception(err)
...
#--------------------------------------------------------------------
    def _check_update_but_in_zone(self, devicename):
        '''
        ...
        try:
            if (self.state_this_poll.get(devicename) != NOT_SET and
                    ...
                    self._check_next_update_time_reached(devicename) is False):

                #log_msg = ("{} Not updated, in zone {}").format(
                #    self._format_fname_devtype(devicename),
                #    self.state_this_poll.get(devicename))
                #self.log_debug_msg(log_msg)
                #event_msg = "__Update cancelled, already in Zone {}".format(
                #    self.state_this_poll.get(devicename))
                #self._save_event(devicename, event_msg)
                return True
...
#--------------------------------------------------------------------
    def _log_device_status_attrubutes(self, status):
        ...
        log_msg = ("►ICLOUD DEVICE STATUS/LOCATION, ●deviceDisplayName={}, "
            ...
            self._timestamp_to_time_utcsecs(location[ATTR_LOC_TIMESTAMP]))
        self.log_debug_msg('*', log_msg)
        return True
        ...
        try:
            for near_devicename_zone in self.waze_distance_history:
                ...
                if dist_from_other_waze_data < test_distance:
                    event_msg = ("__Waze history data used from {}, "
                        "Dist from current location-{}m, Dist from zone-{}km, "
                        "Travel time-{}min").format(
                            near_devicename_zone,
...
#########################################################
    ...
    def log_debug_msg(self, devicename, msg):
        if (self.log_debug_msgs_flag or self.log_debug_msgs_trace_flag):
            _LOGGER.info("◆%s-%s◆ %s", devicename, self.group, msg)
        else:
            _LOGGER.debug("◆%s-%s◆ %s", devicename, self.group, msg)

    def log_debug_interval_msg(self, devicename, msg):
        if 'interval' in self.debug_control:
            _LOGGER.debug("◆%s-%s◆ %s", devicename, self.group, msg)
    ...
    def service_handler_icloud_update(self, group, arg_devicename=None,
        ...
        for devicename in self.tracked_devices:
            ...
            if arg_command_cmd in ('debug', 'info'):
                ...
                if self.next_update_time[devicename_zone] == PAUSED:
...
#--------------------------------------------------------------------
    def service_handler_icloud_setinterval(self, group, arg_interval=None,
        ...
        if arg_devicename and self.CURRENT_TRK_METHOD_IOSAPP:
            if self.iosapp_locate_cnt.get(arg_devicename) > self.max_iosapp_locate_cnt:
                return
